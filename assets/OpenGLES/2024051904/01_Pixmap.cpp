...
main(int argc, char* argv[]) {
  ...
  */
  ...
  gc_vals.foreground = WhitePixel(display, DefaultScreen(display));
  gc_vals.line_width = 3;
  gc_vals.line_style = LineSolid;
  gc_vals.fill_style = FillSolid;
  GC fg = XCreateGC(display, win,
          GCFunction | GCForeground | GCLineWidth | GCLineStyle | GCFillStyle,
          &gc_vals);
 
  unsigned long foreground_color = 0xFF8877; // 绿色
  XSetForeground(display, fg, foreground_color);
  Pixmap pixmap = XCreatePixmap(display, win, width, height, DefaultDepthOfScreen(DefaultScreenOfDisplay(display)));
  XFillRectangle (display, pixmap, fg, 0, 0, width, height);
   ...
   * 
  */
  EGLImageKHR image = eglCreateImageKHR (egl_display, EGL_NO_CONTEXT, EGL_NATIVE_PIXMAP_KHR,  (EGLClientBuffer)pixmap, imageAttributes);
