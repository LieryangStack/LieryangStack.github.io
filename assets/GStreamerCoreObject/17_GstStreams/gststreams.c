...
/**
 * @name: gst_stream_new
 * @param stream_id: （可为NULL）：新流的ID。如果为 %NULL，则会自动生成一个新的ID。
 * @param caps: （可为NULL）（不转移）：流的 #GstCaps
 * @param type: 流的 #GstStreamType
 * @param flags: 流的 #GstStreamFlags
 * @brief: 为给定的 @stream_id、@caps、@type 和 @flags 创建一个新的 #GstStream。
*/
GstStream *
