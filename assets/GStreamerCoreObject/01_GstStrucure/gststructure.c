 ...
 * @title: GstStructure
 * @short_description: 包含名称和值字段的通用结构
 * @see_also: #GstCaps, #GstMessage, #GstEvent, #GstQuery
 *
 * #GstStructure 是一个键/值对的集合。键表示为 GQuarks，值可以是任何 GType。
 *
 * 除了键/值对，#GstStructure 还有一个名称。名称以字母开头，并且可以包含字母、数字以及
 * "/-_.:"。
 *
 * #GstStructure 被 GStreamer 的各种子系统用于以灵活和可扩展的方式存储信息。#GstStructure 
 * 没有引用计数，因为它通常是更高级别对象的一部分，如 #GstCaps、#GstMessage、#GstEvent、#GstQuery。
 * 它通过 gst_structure_set_parent_refcount() 方法提供了一种强制可变性的手段。
 *
 * 可以使用 gst_structure_new_empty() 或 gst_structure_new() 创建 #GstStructure，
 * 这两个函数都接受一个名称和一组可选的键/值对及其值的类型。
 *
 * 可以使用 gst_structure_set_value() 或 gst_structure_set() 更改字段值。
 *
 * 可以使用 gst_structure_get_value() 或更方便的 gst_structure_get_*() 函数检索字段值。
 *
 * 可以使用 gst_structure_remove_field() 或 gst_structure_remove_fields() 移除字段。
 *
 * 结构中的字符串必须是 ASCII 或 UTF-8 编码。不允许使用其他编码。但字符串可能是 %NULL。
 *
 * ## 序列化格式
 *
 * GstStructure 序列化格式序列化 GstStructure 名称、键/GType/值，以逗号分隔列表的形式，
 * 其中结构名称作为第一个字段，后面跟着分隔的键/值对，例如 `key=value`，例如：
 *
 * ```
 * a-structure, key=value
 * ````
 *
 * 如果未明确指定，值的类型将被推断，使用 `(GTypeName)value` 语法，例如以下结构将有一个名为
 * 'is-string' 的字段，其值为字符串 'true'：
 *
 * ```
 * a-struct, field-is-string=(string)true, field-is-boolean=true
 * ```
 *
 * *注意*: 如果没有指定 `(string)`，`field-is-string` 的类型会被推断为布尔值。
 *
 * *注意*: 即使 `gchararray` 是实际的 GType 名称，我们也指定了 `(string)` 类型，因为出于方便，
 * 一些众所周知的类型已被别名或缩写。
 *
 * 为了避免指定类型，您可以给“类型系统”一些提示。例如，要指定一个值为 double，您应该添加一个小数
 * （即 `1` 是一个 `int`，而 `1.0` 是一个 `double`）。
 *
 * *注意*: 当使用 #gst_structure_to_string 序列化结构时，所有值都会显式地进行类型化。
 *
 * 一些类型有特殊的分隔符：
 *
 * - [GstValueArray](GST_TYPE_ARRAY) 用大括号 `{` 和 `}` 包围。例如 `a-structure, array={1, 2, 3}`
 * - 范围使用方括号 `[` 和 `]`。例如 `a-structure, range=[1, 6, 2]`，1 是最小值，6 是最大值，2 是步长。
 *   要指定 #GST_TYPE_INT64_RANGE，您需要显式指定，如：`a-structure, a-int64-range=(gint64) [1, 5]`
 * - [GstValueList](GST_TYPE_LIST) 用“小于和大于”符号 `<` 和 `>` 包围。例如 `a-structure, list=<1, 2, 3>`
 *
 * 结构由空字符 `\0` 或分号 `;` 分隔，后者允许在同一字符串中存储多个结构（见 #GstCaps）。
 *
 * 引号被用作“默认”分隔符，可以围绕任何不使用其他分隔符的类型（例如 `a-struct, i=(int)"1"`）。它们被用来允许
 * 在字符串中添加空格或特殊字符（如分隔符、分号等..），您可以使用反斜杠 `\` 在其中转义字符，例如：
 *
 * ```
 * a-struct, special="\"{[(;)]}\" 可以在引号内使用"
 * ```
 *
 * 它们还允许嵌套结构，例如：
 *
 ...
 * ```
 *
 * 从 1.20 版本开始，嵌套结构和 caps 可以使用方括号 `[]` 指定，例如：
 *
 * ```
 * a-struct, nested=[nested-struct, nested=true]
 * ```
 *
 * > *注意*: 由于向后兼容性原因，gst_structure_to_string() 不会使用该语法，为此目的增加了
