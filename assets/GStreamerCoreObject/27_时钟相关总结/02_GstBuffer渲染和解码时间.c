...
on_pad_probe(GstPad *pad, GstPadProbeInfo *info, gpointer user_data) {
  ...
  GstElement *element = (GstElement *)user_data;

  /* 查看GstBuffer的相关时间 */
  if (GST_PAD_PROBE_INFO_TYPE (info) & GST_PAD_PROBE_TYPE_BUFFER) {

    GstClockTime running_time = gst_element_get_current_running_time (element);
    g_print("running_time: %" GST_TIME_FORMAT "\n", GST_TIME_ARGS(running_time));

    GstBuffer *buf = GST_PAD_PROBE_INFO_BUFFER (info);
    g_print("buf->pts: %" GST_TIME_FORMAT "\n", GST_TIME_ARGS(buf->pts));
    g_print("buf->dts: %" GST_TIME_FORMAT "\n", GST_TIME_ARGS(buf->dts));
    g_print("buf->duration: %" GST_TIME_FORMAT "\n", GST_TIME_ARGS(buf->duration));
    g_print("buf->offset: %ld\n", buf->offset);
    g_print("buf->offset_end: %ld\n\n", buf->offset_end);
  }
...
timeout_cb (gpointer data) {
  ...
  /* 默认GstClock是GST_CLOCK_TYPE_MONOTONIC类型，这是系统运行时间，表示管道是此刻开始播放的 */
...
main(int argc, char *argv[]) {
  ...
  gst_pad_add_probe(sink_pad, GST_PAD_PROBE_TYPE_EVENT_DOWNSTREAM | GST_PAD_PROBE_TYPE_BUFFER, on_pad_probe, video_sink, NULL);
