...
/**
 * @name: gst_meta_api_type_register
 * @brief: 注册@api类型，把@tags传入的字符串标签都添加到该类型中
 * @param api: 要进行注册的类型名称
 * @param tags: 标签列表，最后以NULL结尾
 * @return: 返回注册类型在类型系统中的值
*/
GType
gst_meta_api_type_register (const gchar * api, const gchar ** tags)
{
  ...
  /* 本质上调用的是g_type_register_static函数，注册的时候只注册了名称，GTypeInfo全为空 */
