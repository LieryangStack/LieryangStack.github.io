...
/* 获取与 @type 相关联的 #GstEventTypeFlags。 */
GstEventTypeFlags
...
gst_event_init (GstEventImpl * event, GstEventType type)
{
  ...
  GST_EVENT_TIMESTAMP (event) = GST_CLOCK_TIME_NONE; /* 默认事件时间戳没有时间 */
...
/**
 * @name: gst_event_new_custom
 * @param type: 事件的类型
 * @param structure: 事件拥有的结构体
 * @brief: 根据@type创建一个Event，#GstEventType中的特定事件Element会有特定的处理函数。
 *         这个函数也可以用于创建没有特定处理函数的事件。
*/
GstEvent *
