...
main (int argc, char *argv[]) {
  ...
  if (!data.pipeline || !data.source || !data.h265depay || !data.h265parse \
      || !data.h265decode || !data.video_convert || !data.video_sink \
  ...
  gst_bin_add_many (GST_BIN (data.pipeline), data.source, 
      data.h265depay, data.h265parse, data.h265decode, data.video_convert, data.video_sink, \
      data.rtpmp4gdepay, data.aacparse, data.avdec_aac, data.audio_convert, data.audio_sink, NULL);


  if (!gst_element_link_many (data.h265depay, data.h265parse, \
                              data.h265decode, data.video_convert, data.video_sink, NULL)) {
