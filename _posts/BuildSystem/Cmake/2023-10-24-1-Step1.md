---
layout: post
title: Step1：A Basic Starting Point
categories: CMake
tags: [CMake]
---
...
还请注意，教程中的每一步都是基于下一步构建的。例如，下一章的起始代码是上一章cm的完整解决方案。

## 1 Exercise - Building a Basic Project

最基本的CMake项目是通过单个源代码文件构建的可执行文件。对于这样简单的项目，只需一个包含<font colo="red">三个命令的CMakeLists.txt文件</font>即可。

注意：虽然CMake支持大写、小写和大小写混合的命令，但推荐使用小写命令，并将在整个教程中使用小写命令。

1. `cmake_minimum_required()`：命令指定最低CMake版本(任何项目的顶层CMakeLists.txt必须使用)。这会建立策略设置，并确保以下CMake函数在兼容的CMake版本上运行。

2. `project()`：命令来设置项目名称。这个调用在每个项目中都是必需的，并应该在cmake_minimum_required()之后尽快调用。正如我们将在后面看到的，这个命令也可以用于指定其他项目级别的信息，如语言或版本号。

3. `add_executable()`：命令告诉CMake使用指定的源代码文件创建一个可执行文件。
...
## 2 Exercise - Specifying the C++ Standard
...
## 3 Exercise - Adding a Version Number and Configured Header File
