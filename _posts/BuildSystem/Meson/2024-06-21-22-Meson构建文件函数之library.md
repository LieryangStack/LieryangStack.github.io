---
layout: post
title: 二十二、Meson构建文件函数——library()
categories: Meson
tags: [Meson]
---

- `library` 可以构建一个库，该库可以是静态的、共享的或两者兼有的，这取决于 `default_library` 用户选项值。大多数情况下，你应该使用这个方法，而不是shared_library()、static_library()或both_libraries()。
...
project('glib', 'c',
  ...
  default_options : [
    ...
    'default_library=both' # shared, static, both
  ]
)
```
